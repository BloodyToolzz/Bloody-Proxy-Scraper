...
file.write('# Proxies Scraped by: https://github.com/BloodyToolzz/Bloody-Proxy-Scraper\n')
file.write('# Time started: ' + now.strftime("%Y/%m/%d %H:%M:%S\n\n"))
file.close()
...
def start(url):
    try:
        ...
        pattern_one(req)
        Write.Print(f' [+] Scraped {len(good_proxies)} proxies from: {url}\n', Colors.green_to_white, interval=0)
    except requests.exceptions.SSLError: Write.Print(str(url) + ' [x] SSL Error\n', Colors.purple_to_blue, interval=0)
    except: Write.Print(str(url) + ' [x] Random Error\n', Colors.red_to_yellow, interval=0)
...
scraping_time = (time.time() - startTime)
Write.Print("\n\nEnded at: " + now.strftime("%Y/%m/%d %H:%M:%S\n"), Colors.purple_to_blue , interval=0)
Write.Print(f'[/] Total scraped proxies: ({len(good_proxies)})\n', Colors.purple_to_blue, interval=0)
Write.Print(f'Scraped in {scraping_time}ms\n', Colors.purple_to_blue, interval=0)
Write.Input('Press any key to continue . . .', Colors.purple_to_blue, interval=0)
